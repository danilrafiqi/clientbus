...
class AgenAdd extends Component {
  ...
  detail = id => {
    axios.get(`${process.env.REACT_APP_API}/tujuan/${id}`).then(res => {
      this.setState({
        open: true,
        pemberangkatan: res.data[0].pemberangkatan,
        pemberhentian: res.data[0].pemberhentian,
        rute_id: res.data[0].rute_id
      });
    });
  };
  update = id => {
    axios
      .put(`${process.env.REACT_APP_API}/tujuan/${id}`, {
      ...
      <AddUpdate
        ...
        <form className={classes.root} autoComplete="off">
          ...
          <FormControl
            ...
            fullWidth>
            <InputLabel htmlFor="rute_id">Rute</InputLabel>
            <Select
